...
Twinkle.tag.redirectList = {
	'Grammar, punctuation, and spelling': {
		'Abbreviation': [
			...
			{ tag: 'letter–word combination', description: 'redirect from a title that is a letter–word combination to a topic that describes or explains the combination', from: true },
			...
			{ tag: 'systematic name', description: 'redirect from a systematic name to a more commonly used name', from: true, to: true },
		],
		'Capitalisation': [
	...
	'Alternative names': {
		General: [
			...
			{ tag: 'identifier', description: 'redirect from a unique (alpha-)numerical identifier (like an EAN) to an article discussing that particular class of identifiers in mainspace', from: true },
			...
			{ tag: 'numerals', description: 'redirect from a title that includes the mathematical symbol of a number (or numbers) to an article with the word form of the number', from: true, to: true },
			...
			{ tag: 'title without diacritics', description: 'redirect from a page name that does not have diacritical marks (accents, umlauts, etc.)', from: true },
		],
		People: [
			...
			{ tag: 'name with title', description: 'redirect from a person\'s name preceded or followed by a title to the name with no title or with the title in parentheses', from: true, to: true },
		...
		Technical: [
			...
			{ tag: 'file extension', description: 'redirect from a title that is a file extension (such as .svg and .txt)', from: true },
			...
			{ tag: 'H2 symmetry', description: 'edirect from an H2 symmetry to a section of an article about a hyperbolic tiling on its symmetry', from: true },
			...
			{ tag: 'manufacturer\'s design', description: 'redirect from a manufacturer\'s designation to a more common military or commercial designation of an aircraft, vehicle, or other item', from: true },
			{ tag: 'reporting name', description: 'redirect from a NATO reporting name to the official designation of Soviet, Russian or Chinese military equipment', from: true },
			{ tag: 'S2 symmetry', description: 'redirect from an S2 symmetry to an article or section of an article of a spherical polyhedron on its symmetry', from: true },
			{ tag: 'stock symbol', description: 'redirect from a stock symbol to the company it represents on a stock exchange', from: true },
			{ tag: 'symbol', description: 'redirect from a symbol to the meaning of the symbol or to a related topic', from: true },
			{ tag: 'technical name', description: 'redirect from a more technical name to a common name or vise versa', from: true, to: true },
			{ tag: 'trademark', description: 'redirect from a trademark to a more common stylisation or to the owner of the trademark', from: true },
			{ tag: 'US military aircraft designator', description: 'redirect from a title that is a US military aircraft designator to an article about the corresponding aircraft or missile', from: true },
			{ tag: 'variant military designation', description: 'redirect from a variant military designation to the parent type of an aircraft, vehicle, or other item', from: true },
		],
		Organisms: [
			{ tag: 'animal', description: 'redirect from one or more animals to a related main topic that describes the animal(s)', from: true },
			{ tag: 'scientific name', description: 'redirect from the common name to the scientific name or vise versa', from: true, to: true },
			...
			{ tag: 'monotypic taxon', description: 'redirect from the only lower-ranking member of a monotypic taxon to its monotypic taxon or vise versa', from: true, to: true },
			{ tag: 'plant with possibilities', description: 'redirect from a plant-related title that potentially could be expanded into an article', from: true },
		...
		Geography: [
			...
			{ tag: 'former Swiss municipality', description: 'redirect from a title for a former Swiss municipality to the municipality into which it was incorporated', from: true },
			{ tag: 'highway', description: 'redirect from a highway page title that may be a topic more detailed than the target page', from: true },
			{ tag: 'highway in region', description: 'redirect from the format "(highway) in (region)" to the (highway) article', from: true },
			{ tag: 'highway in region without possibilities', description: 'redirect from the format "(highway) in (region)" to the (highway) article, without possibilites', from: true },
			{ tag: 'London place name', description: 'redirect from a London place name to an article about that place', from: true },
			...
			{ tag: 'route number', description: 'redirect from a numbered road that is insufficiently notable for an independent article to a list of roads that covers the route', from: true },
			{ tag: 'Swiss municipality 2L', description: 'redirect from a title in the form "[name of municipality](space)[two-letter abbreviation for canton]"', to: true },
			{ tag: 'Swiss municipality (canton)', description: 'This is a redirect from a title in the form "[name of municipality]space[(name of canton)]"', to: true },
			{ tag: 'Swiss municipality (, Switzerland)', description: 'redirect from a title in the form "[name of municipality], Switzerland"', to: true },
			{ tag: 'telephone number', description: 'redirect from a telephone number to a related topic, such as the company that can be reached there', from: true },
			{ tag: 'UN/LOCODE', description: 'redirect from a title that is a UN/LOCODE geographic code to an article or page associated with that code', from: true },
			{ tag: 'US postal abbreviation', description: 'redirect from a US postal abbreviation to its associated municipality', from: true },
			{ tag: 'UK postcode', description: 'redirect from a United Kingdom postcode (postal code) to its assigned geographic area', from: true },
		],
		Other: [
			...
			{ tag: 'Greek organizational letters', description: 'redirect from the initialism (usually Greek letter Unicode symbols) for a Greek letter organization (sorority, fraternity, honor society, etc.)', from: true},
			{ tag: 'IMO number', description: 'redirect from a title that is an IMO number to an article about the corresponding ship, ship owner or management company', from: true},
			{ tag: 'imprint', description: 'from a publisher\'s imprint to a related article that gives information about the imprint', from: true},
			{ tag: 'predecessor company name', description: 'redirect from the name of a former company to a successor company', from: true },
			{ tag: 'railroad name with ampersand', description: 'a redirect from the title of a railroad that is correct, but uses an ampersand (&) in place of the word "and"', from: true },
			{ tag: 'South Indian spelling', description: 'redirect from a term in South Indian spelling to an alternate spelling variation', from: true },
			{ tag: 'Twitter username', description: 'redirect from a Twitter username to its subject\'s article in mainspace. Every redirect title in this category must begin with an "@"', from: true },
			{ tag: 'Wade–Giles romanization', description: 'redirect from the Wade–Giles spelling of a Chinese word or name to the Pinyin or other preferred spelling', from: true },
		]
	},
	'Navigation aids': {
		'Navigation': [
			...
			{ tag: 'high-use template', description: 'redirect from a highly-transcluded redirect to a template', from: true },
			...
			{ tag: 'redirect template', description: 'redirect from an alias/shortcut (usually), or from a redirect that is the result of a page move/rename', to: true },
			{ tag: 'restricted', description: 'redirect from a technically restricted page name to a page titled as closely as possible to the actual and true subject name' },
			...
			{ tag: 'stub template', description: 'redirect from a template shortcut or title that may have been moved (renamed) or merged to the target stub template', to: true },
			{ tag: 'subpage', description: 'redirect to a subpage', from: true, to: true },
			{ tag: 'test purposes only', description: 'redirect from an rcat that is for test purposes only' },
		],
		'Disambiguation': [
		...
		'Merge, duplicate & move': [
			...
			{ tag: 'userbox', description: 'edirect from a shortcut or the result of a page move/rename to a userbox template', to: true },
			{ tag: 'warning template', description: 'redirect from a user warning template shortcut or convenience redirect to a user warning template', to: true },
	...
	'Media': {
		...
		Fiction: [
			...
			{ tag: 'ME from former name', description: 'redirect from a former name or working title of a Middle-earth related topic' },
			{ tag: 'ME to list entry', description: 'redirect from the name of a Middle-earth "minor topic" to a list article' },
			{ tag: 'ME to section', description: 'redirect from the name of a Middle-earth "minor topic" to a section' },
			{ tag: 'ME with possibilities', description: 'redirect from the name of a Middle-earth topic which currently redirects to a section or to an entry in a list or other article, and which may be expandable into a separate article' },
	...
	'Miscellaneous': {
		'Related information': [
			...
			{ tag: 'military designation', description: 'redirect from a military designation to a more common civilian or manufacturer\'s designation of an aircraft, vehicle, or other item', from: true },
			{ tag: 'product name', description: 'redirect from a title that is the name of a product or service to a more general, relevant article', from: true },
			{ tag: 'quotation', description: 'redirect from a quotation to its best-known source', from: true },
			...
			{ tag: 'slogan', description: 'redirect from a slogan (or motto) to an article or section of an article about the slogan', from: true },
			...
			{ tag: 'scouting from merge', description: 'redirect from a Scouting title that was merged into the target article' },
			{ tag: 'school', description: 'redirect from a school article that had very little information', from: true },
			{ tag: 'subsidiary title', description: 'edirect from a subsidiary title in the peerage system to the correct title', from: true },
			{ tag: 'subtopic', description: 'redirect from a title that is a subtopic of the target article', from: true, to: true },
			{ tag: 'sports league', description: 'redirect from team or other redirect currently not with its own article to article for the sports league', to: true },
			...
			{ tag: 'writer', description: 'redirect from a writer (or writers) to a related topic usually about one of their works', from: true },
			{ tag: 'year', description: 'redirect from a formatted year title to the related year article', from: true },
		],
		'With possibilities': [
